...
### 2. Проверка обложки и оглавления

- **Обложка:** Убедитесь, что в архиве присутствует файл `titlepage.xhtml`, содержащий ссылку на изображение в теге `<image xlink:href="...">`, либо файл обложки в форматах `.jpg`, `.jpeg`, `.png` (бот проверит их автоматически).

- **Оглавление (`toc.ncx`):** Для корректного подсчета глав бот сканирует файл с расширением `.ncx` и анализирует элементы `<navPoint>`. Убедитесь, что файл оглавления присутствует и содержит корректные теги `<text>` с названиями глав.
